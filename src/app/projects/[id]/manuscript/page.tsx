...
const buildDefaultCopyrightText = (authorName?: string) => {
  const author = authorName?.trim() || "the author";

  return `All rights reserved. No part of this publication may be reproduced, distributed, or transmitted in any form or by any means, including photocopying, recording, uploading, sharing, or other electronic or mechanical methods, without the prior written permission of ${author}, except as permitted by law.

Written by ${author}.
Published with WriteNowBooks.com.`;
};

const buildDefaultBackCoverBlurb = (
  project?: SavedOrder | null,
  pages?: ProjectPages | null
) => {
  const title =
    project?.bookData?.topic?.trim() ||
    project?.bookData?.bookType?.trim() ||
    "this book";
  const author = project?.bookData?.authorName?.trim() || "the author";
  const audience = project?.bookData?.audience?.trim() || "readers";
  const tone = project?.bookData?.tone?.trim() || "engaging";
  const bookType = project?.bookData?.bookType?.trim() || "book";
  const topic = project?.bookData?.topic?.trim() || title;
  const firstPage = pages?.pages?.[0]?.text?.trim();

  if (bookType.toLowerCase().includes("children") || pages?.pages?.length) {
    return `${title} is a ${tone.toLowerCase()} children’s book created for ${audience}. This story invites young readers into a warm, imaginative experience built around ${topic}.

With simple page-by-page storytelling and a clear visual flow, this book is designed to help children enjoy reading, build confidence, and connect with the story from beginning to end.${
      firstPage ? `

The story begins: “${firstPage.slice(0, 180)}${firstPage.length > 180 ? "..." : ""}”` : ""
    }

Written by ${author}.`;
  }

  return `${title} is a ${tone.toLowerCase()} ${bookType.toLowerCase()} created for ${audience}. This book explores ${topic} in a clear, organized way that helps readers understand the message, connect with the content, and take something meaningful from the experience.

Inside, readers will find a structured flow, practical insight, and a complete reading experience from beginning to end.
...
export default function ManuscriptPage() {
  ...
  useEffect(() => {
    ...
    const generatedCopyrightText = buildDefaultCopyrightText(
      foundProject?.bookData?.authorName
    );

    const defaultFrontMatterForProject: FrontMatter = {
      ...DEFAULT_FRONT_MATTER,
      publisherName: "WriteNowBooks.com",
      copyrightText: generatedCopyrightText,
    };

    if (savedFrontMatter) {
      setFrontMatter({
        ...defaultFrontMatterForProject,
        ...savedFrontMatter,
        copyrightText: savedFrontMatter.copyrightText?.trim()
          ? savedFrontMatter.copyrightText
          : generatedCopyrightText,
      });
    } else {
      setFrontMatter(defaultFrontMatterForProject);
      localStorage.setItem(
        `writeNowFrontMatter_${id}`,
        JSON.stringify(defaultFrontMatterForProject)
      );
    }
    ...
    const generatedBackCoverBlurb = buildDefaultBackCoverBlurb(
      foundProject || null,
      savedPages
    );

    const defaultPublishingSettingsForProject: PublishingSettings = {
      ...DEFAULT_PUBLISHING_SETTINGS,
      spineText:
        foundProject?.bookData?.topic || foundProject?.bookData?.bookType || "",
      backCoverBlurb: generatedBackCoverBlurb,
    };
    ...
    if (savedPublishingSettings) {
      ...
      setPublishingSettings({
        ...defaultPublishingSettingsForProject,
        ...savedPublishingSettings,
        backCoverBlurb:
          savedPublishingSettings.backCoverBlurb?.trim() &&
          savedPublishingSettings.backCoverBlurb !== oldPlaceholder
            ? savedPublishingSettings.backCoverBlurb
            : generatedBackCoverBlurb,
      });
    } else {
      setPublishingSettings(defaultPublishingSettingsForProject);
      localStorage.setItem(
        `writeNowPublishingSettings_${id}`,
        JSON.stringify(defaultPublishingSettingsForProject)
      );
    }
  ...
  const hasExportContent = () => {
    ...
    return !!chapters && chapters.chapters.length > 0;
  };
...
const getTrimSizeMM = () => {
  ...
  const getKDPReadyIssues = () => {
    ...
    const exportStyle = getExportBookStyle();

    if (exportStyle !== "children" && (!chapters || chapters.chapters.length === 0)) {
    ...
    if (exportStyle === "children" && (!pages || pages.pages.length === 0)) {
...
Written by ${author}.`;
  ...
  const updatePageText = (pageNumber: number, value: string) => {
    if (!pages) return;

    const updated = {
      ...pages,
      pages: pages.pages.map((page) =>
        page.pageNumber === pageNumber ? { ...page, text: value } : page
      ),
    };
  ...
  const saveEditedManuscript = () => {
    ...
    if (isChildrenBook() && pages) {
      ...
      setSaveMessage("Children’s book page edits saved successfully.");
      return;
    }
...
const createPDFBlob = async () => {
  ...
  const createDOCXBlob = async () => {
    ...
    if (style !== "children" && !chapters) return null;
  ...
  const handleExportPDF = async () => {
    if (!project || !hasExportContent()) {
      setSaveMessage("There is no manuscript content ready to export yet.");
      return;
    }
    ...
    try {
      ...
      if (!blob) {
        setSaveMessage("PDF export could not be created.");
        return;
      }
  ...
  const handleExportDOCX = async () => {
    ...
    try {
      ...
      if (!blob) {
        setSaveMessage("DOCX export could not be created.");
        return;
      }
  ...
  const handleExportCoverWrap = async () => {
    ...
    try {
      ...
      if (!blob) {
        setSaveMessage("Cover wrap could not be created.");
        return;
      }

      markExportChecklistReady("cover");
      saveAs(blob, `${sanitizeFilename(getTitle())}-full-cover-wrap.pdf`);
      setSaveMessage("Cover wrap downloaded successfully.");
    } catch (error) {
      console.error("Cover wrap export error:", error);
      setSaveMessage(
        error instanceof Error ? error.message : "Cover wrap export failed."
      );
    } finally {
  ...
  const handleExportZIP = async () => {
    if (!project || !hasExportContent()) {
      setSaveMessage("There is no manuscript content ready to package yet.");
      return;
    }
...
`
  ...
  const exportReady = hasExportContent();
  const canSaveEdits = childrenBookMode ? !!pages : !!chapters;
        ...
        <div className="mb-10 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            <button
              ...
              disabled={!exportReady || zipLoading}
                ...
                return (
                  ...
                  >
                    ...
                    <textarea
                      value={page.text}
                      onChange={(e) =>
                        updatePageText(page.pageNumber, e.target.value)
                      }
                      className="w-full min-h-[120px] border border-gray-300 rounded-lg px-4 py-4 text-xl leading-8 text-center font-semibold"
                    />
                  </div>
                );
              })}
